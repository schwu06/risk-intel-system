...
2. 「风险类别」只能从以下五类中选择：信贷风险、市场风险、流动性与资产负债、合规与反洗钱、国别与地缘。必须填写一个主类型，最多可增加一个辅类型；格式为“主类型”或“主类型｜辅类型”。主类型必须是银行最直接面对的风险：违约、重组、评级/展望、现金流或减值归信贷；利率、汇率、股债或商品价格波动归市场；融资冻结、存款流失、利差、债券发行受阻或期限错配归流动性与资产负债；制裁、罚单、许可、AML/KYC、出口管制归合规与反洗钱；战争、航道/港口/管道中断、资本管制、国有化归国别与地缘。只有价格变动不能归信贷；制裁或监管行动已落地时主类型改为合规与反洗钱；战争或航道消息尚未点名偿债后果时归国别与地缘。
