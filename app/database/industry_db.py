...
def _open_industry_session(sector_key: str) -> tuple[Session, object]:
    init_industry_database(sector_key)
    session = get_industry_sessionmaker(sector_key)()
    token = current_industry_sector.set(sector_key)
...
def get_industry_db(sector_key: str) -> Generator[Session, None, None]:
    try:
        ...
        raise HTTPException(status_code=400, detail="无效行业分类") from exc
    session, token = _open_industry_session(sector_key)
    try:
        yield session
    finally:
        current_industry_sector.reset(token)
        session.close()
...
def get_industry_db_with_query(
    ...
    session, token = _open_industry_session(key)
    try:
        yield session
    finally:
        current_industry_sector.reset(token)
...
def open_industry_session(sector_key: str) -> Session:
    """供页面路由同步调用；调用方负责 close 或 reset context。"""
    require_sector_key(sector_key)
    session, _token = _open_industry_session(sector_key)
    return session
